...
test_that("throws error for unsupported type", {
  expect_error(vec_all_equal(expression(x), 1), class = "vctrs_error_scalar_type")
})

test_that("correct behaviour for basic vectors", {
  expect_true(vec_all_equal(c(TRUE, TRUE), c(TRUE, TRUE)))
  expect_true(vec_all_equal(c(2L, 2L), c(2L, 2L)))
  expect_true(vec_all_equal(c(2, 2), c(2, 2)))
  expect_true(vec_all_equal(c("1", "1"), c("1", "1")))
  expect_true(vec_all_equal(list(1:3, 1:3), list(1:3, 1:3)))
  expect_true(vec_all_equal(as.raw(c(1, 1)), as.raw(c(1, 1))))
  expect_true(vec_all_equal(c(1, 1) + 2i, c(1, 1) + 2i))

  expect_false(vec_all_equal(c(TRUE, TRUE), c(TRUE, FALSE)))
  expect_false(vec_all_equal(c(1L, 1L), c(1L, 2L)))
  expect_false(vec_all_equal(c(1, 1), c(1, 2)))
  expect_false(vec_all_equal(c("1", "1"), c("1", "2")))
  expect_false(vec_all_equal(list(1, 1), list(1, 1:3)))
  expect_false(vec_all_equal(as.raw(c(1, 1)), as.raw(c(1, 2))))
  expect_false(vec_all_equal(c(1, 1) + 2i, c(1, 2) + 2i))
})

test_that("correct behavior for size 0 input", {
  expect_true(vec_all_equal(integer(), integer()))
  expect_true(vec_all_equal(list(), list()))
  expect_true(vec_all_equal(new_data_frame(), new_data_frame()))
})

test_that("`na_equal` works", {
  expect_equal(vec_all_equal(c(TRUE, NA), c(TRUE, NA)), NA)
  expect_equal(vec_all_equal(c(1L, NA_integer_), c(1L, NA_integer_)), NA)
  expect_equal(vec_all_equal(c(1, NA_real_), c(1, NA_real_)), NA)
  expect_equal(vec_all_equal(c("1", NA_character_), c("1", NA_character_)), NA)
  expect_equal(vec_all_equal(list(1, NULL), list(1, NULL)), NA)

  expect_true(vec_all_equal(c(TRUE, NA), c(TRUE, NA), na_equal = TRUE))
  expect_true(vec_all_equal(c(1L, NA_integer_), c(1L, NA_integer_), na_equal = TRUE))
  expect_true(vec_all_equal(c(1, NA_real_), c(1, NA_real_), na_equal = TRUE))
  expect_true(vec_all_equal(c("1", NA_character_), c("1", NA_character_), na_equal = TRUE))
  expect_true(vec_all_equal(list(1, NULL), list(1, NULL), na_equal = TRUE))
})
...
test_that("data frames are compared row wise", {
  ...
  expect_true(vec_all_equal(df1, df1))
  expect_false(vec_all_equal(df1, df2))
})

test_that("`na_equal` works with data frames", {
  df1 <- data_frame(x = c(1, 1), y = c("a", NA_character_))

  expect_equal(vec_all_equal(df1, df1), NA)
  expect_true(vec_all_equal(df1, df1, na_equal = TRUE))
})
...
test_that("the equality proxy is taken recursively", {
  ...
  expect_equal(vec_all_equal(df, df), TRUE)
  expect_equal(vec_all_equal(df, df2), FALSE)
})
...
test_that("can compare data frames with various types of columns", {
  ...
  expect_false(vec_all_equal(x1, y1))
  expect_false(vec_all_equal(x2, y2))
  expect_false(vec_all_equal(x3, y3))
  expect_false(vec_all_equal(x4, y4))
  expect_false(vec_all_equal(x5, y5))
  expect_false(vec_all_equal(x6, y6))
})
...
test_that("can compare data frames with list columns", {
  ...
  expect_equal(vec_all_equal(df1, df1), TRUE)
  expect_equal(vec_all_equal(df1, df2), FALSE)
})
...
test_that("can compare lists of expressions", {
  ...
  expect_equal(vec_all_equal(x, x), TRUE)
  expect_equal(vec_all_equal(x, y), FALSE)
})

test_that("can catch if one of the inputs is NULL", {
  expect_error(vec_all_equal(NULL, 1), "must have the same type")
  expect_error(vec_all_equal(NULL, 1), "must have the same type")
})

test_that("error when both inputs are NULL", {
  expect_error(vec_all_equal(NULL, NULL), "Unimplemented type")
})
